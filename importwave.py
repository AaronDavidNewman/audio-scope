...
xinv = np.concatenate((xar[256:512:], xar[0:256]))
...
def fft_plot(audio, sample_rate):
  ...
  print(f'window count is {window_count}')
  ...
  for x in range(0, window_count):
    ...
    for subplt in range(0, bunch):
      ...
      # use sqrt so the dots and up being close to the same size for scatter plot
      ...
      for x_freq in range(0,imagesize):
        ...
        yval = y_freq[x_index] * (imagesize / max_freq) * xenergy[x]
        yar.append(yval)
        yinv.append(-1.0 * yval)
      ...
      plotPolarScatterArray(axis=ax, xarray=xinv, yarray=yar, colors=colors, sizes=counts)
